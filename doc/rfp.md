...
| データ | ソース | ライセンス | 対応 |
|---|---|---|---|
| 単語・ランク（Level 1-8） | JACET8000 | 著作権：JACET保有。権利者が定める利用条件を確認のうえ、その範囲内で利用する（本アプリは非商用・教育目的） | アプリ内クレジット表示 |
| 定義・品詞・発音記号・活用形・例文・コロケーション | Wiktionary API | CC BY-SA 4.0 | アプリ内クレジット表示、改変部分もCC BY-SA継承 |
| 学習アルゴリズム | SM-2（SuperMemo） | 出典元（SuperMemo）が定める利用条件を確認のうえ利用し、規定に従い著作権表示を行う | アプリ内クレジット表示必須 |

> **利用条件に関する注記**: 本表のライセンス欄は実装時の取り扱い方針であり、法務判断を確定するものではない。各データソースの最新の利用条件・ライセンス規定を権利者の公表情報で確認し、その範囲内で利用すること。疑義がある場合は権利者へ確認する。
...
> **カバー率について**: 「カバー率」とは、一般的な英文（新聞・書籍・会話等）に登場する語のうち、当該レベルまでを習得した場合に理解できる語の割合の目安を指す。JACET8000 は使用頻度順に配列されているため、上位レベルほど1語あたりの出現頻度が高く、少ない語数で高いカバー率に到達する。以下の数値は一般的な語彙カバー率研究に基づく**目安（仮定）**であり、確定値ではない。正確な数値は今後の実データ検証で差し替え可能とする。
...
```
※ Piper TTS・flutter_tts のいずれも、単語表示のたびにテキストから**実行時にリアルタイム合成して再生**する方式とする。単語ごとの音声ファイルを事前生成・同梱・キャッシュすることはしない。ただし実機での実行時合成レイテンシが許容できない場合に限り、`words.audio_file_path` を用いた事前生成方式へ切り替える余地を残す（第7章・第9章参照）。

---

## 5. SM-2アルゴリズム仕様

本アプリのSM-2は標準SM-2（SuperMemo）をベースに、4段階評価（◎〇△×）と本アプリ独自の初回interval設定を組み合わせた実装とする。以下の増減値は標準SM-2に準拠し、本アプリの確定仕様とする。
...
各評価押下時、対象単語の `user_progress` レコードを以下の手順で更新する。基準時刻 `now` は端末ローカル日付の当日（0:00 境界）とする。

**(0) レコードの取得または新規作成（未学習単語の初回評価）**
- 新規学習セッションで初めて評価される単語は `user_progress` にレコードが存在しない（第6章のとおり、1語につき進捗レコードは0/1件）。
- 対象単語の `word_id` で `user_progress` を検索し、**レコードが存在しない場合は初回評価の時点で新規作成**する。作成時の初期値は次のとおり（第6章の `DEFAULT` 値と一致）:
  - `ease_factor = 2.5`、`interval_days = 0`、`repetitions = 0`、`fail_count = 0`、`last_grade = NULL`、`last_reviewed_at = NULL`、`next_review_at = NULL`
- この初期値のレコードに対して、以下の (1)〜(6) のSM-2更新を同一トランザクション内で適用する。すなわち未学習単語も「初期値レコードを作成してから更新する」統一フローで処理し、既存レコードの場合は取得したレコードにそのまま (1)〜(6) を適用する。
...
**(5) 苦手単語カウント**
- ×押下時のみ `fail_count += 1`（累計。復習で再度×でも加算する）。

**(6) last_grade**
- 押下した評価値（◎〇△×）で上書き保存する（進捗率は常に**最新評価**のみで算出）。

### 5.3 更新擬似コード

```pseudo
function gradeWord(word_id, level, grade, session_type, now):
    # (0) レコード取得 or 未学習単語なら初期値で新規作成
    progress = user_progress.find(word_id)
    if progress == null:
        progress = user_progress.create(
            word_id          = word_id,
            ease_factor      = 2.5,
            interval_days    = 0,
            repetitions      = 0,
...
function applyGrade(progress, grade, now):
    ...
    save(progress)   # 未学習単語の場合は(0)で作成した初期値レコードがここで確定保存される
    # ※ study_log への記録は呼び出し元 gradeWord() が担う（重複記録を避ける）
```
...
評価値：◎=1.0、〇=0.5、△=0.1、×・未学習=0
```
- 各単語について集計に用いるのは **最新評価（last_grade）のみ**。過去の評価履歴（study_log）は進捗率算出に含めない。
...
集計処理（復習予定・進捗率・苦手単語TOP5・学習量推移）を最大8000語（1000語×8レベル）規模で実用的な応答性能とするため、以下のインデックスを付与する。
...
```
（`user_progress.word_id` は `UNIQUE` 制約により暗黙的にインデックス化されるため、単語単位の進捗参照は追加インデックス不要。）
...
- **音声方式（TTSエンジン切替）**: 初期状態は **flutter_tts（OS標準TTSによる実行時生成）**。設定画面（4.5）から **Piper TTS（オフラインMLベースの事前学習モデルによる高品質音声）** への切替が可能。
  ...
  - **音声合成方式**: flutter_tts・Piper TTS のいずれも、単語表示のたびに**実行時にオンデバイスでテキストから都度リアルタイム合成して再生**する。単語ごとの音声ファイルを事前生成・同梱・キャッシュすることはしない。
  - `words.audio_file_path` は事前生成音声ファイル用の**予約カラム**として残す（初期版では常にNULL）。実機検証で実行時合成のレイテンシがユーザー体験上許容できないと判明した場合に限り、事前生成キャッシュ方式へ切り替えるための将来用カラムとして機能させる（切替方針は第9章の実装方針を参照）。
...
本アプリの主要な設計判断（レベル定義・画面仕様・SM-2式・音声方式・DB設計）は第2〜7章の本文で確定済みである。本章は初期バージョンのスコープ外事項と、実装時に適用する補足方針を記す。

**スコープ外**
- 単語データ（JACET8000 + Wiktionary補完）の収集・変換スクリプトの実装。本アプリは事前収集済みの静的データを同梱する前提であり、収集フロー自体は本RFPのスコープ外（別途進行）とする。
- クラウド同期・アカウント機能（第7章のとおり本バージョン対象外）。

**実装方針**
- **Piper TTSモデルの取得**: 既存の公開インフラ（GitHub Releases / Hugging Face）で配布される英語音声モデル（`.onnx` 形式）を実装時に選定して直接取得する。自前のホスティングは行わない（第4.5節・第7章）。
- **ダウンロードファイルの検証**: 初期版はチェックサムによる改ざん検証を実装しない（公開インフラの配布物をそのまま利用する方針とする）。
- **音声合成方式**: flutter_tts・Piper TTS ともに実行時オンデバイス合成で提供する（第7章・第4.5節で確定）。実機検証で単語表示ごとの合成レイテンシがユーザー体験上許容できないと判明した場合に限り、`words.audio_file_path`（予約カラム）を用いた事前生成キャッシュ方式へ切り替える。
- **集計性能**: 苦手単語TOP5・復習予定・進捗率・学習量推移の集計は最大8000語規模を対象とし、第6章「インデックス」で定義したインデックスにより実用的な応答性能を確保する。
...
**SM-2・データ更新（5章・6章）**
- [ ] 未学習単語（`user_progress` レコード無し）を初めて評価すると、初期値（ease_factor=2.5／interval_days=0／repetitions=0／fail_count=0／last_grade=NULL）でレコードが新規作成され、同一処理内でSM-2更新が適用される。
...
| v1.1 | 2026-07-02 | 完成版。レベル定義に各LV（LV1〜8）の詳細説明を追加。SM-2仕様をease_factor増減値・interval算出式・repetitions規則・next_review_at算出まで実装可能な精度で規定し、レコード新規作成を含む更新擬似コードを明記。音声方式を実行時オンデバイス合成で確定し、`words.audio_file_path` を将来の事前生成用予約カラムとして定義。設定画面（4.5節）とTTSエンジン切替仕様（flutter_tts / Piper TTS）を追加。画面遷移図・学習セッション状態遷移図・DB ER図をMermaidで整備。DBスキーマにインデックス定義と `user_progress.word_id` の `UNIQUE` 制約を追加。用語集・受け入れ基準を新設。 |
