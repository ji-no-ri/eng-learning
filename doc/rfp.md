...
- **音声方式（TTSエンジン切替）**: 初期状態は **flutter_tts（OS標準TTSによる実行時生成）**。設定画面（4.5）から **Piper TTS（オフラインMLベースの事前学習モデルによる高品質音声）** への切替が可能。
  ...
  - `words.audio_file_path` は事前生成音声ファイル用の予約カラムとして残すが、初期実装ではPiper TTSも実行時にオンデバイスでテキストから音声合成する方式（都度生成）とし、単語ごとの音声ファイルを事前生成・同梱することはしない（`assumption`。単語ごとの事前生成キャッシュは将来の最適化余地とする）。
...
本章は、本RFPのスコープ外、または実装段階で確定・検証すべき残課題のみを記す。レベル詳細説明・音声方式・SM-2式・設定画面など主要な設計判断は、それぞれ第3.1節／第7章・第4.5節／第5章／第4.5節の本文で確定済みである。
