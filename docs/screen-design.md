...
LV詳細画面（§6）に表示する「復習予定」の算出・表示仕様を定義する（RFP 4.2 の項目3）。当該LVに限定せず、常に**全レベル横断（全単語）**の数字を用いる点が本項目の要点である。

- **明日の復習予定数**: `next_review_at` の端末ローカル日付が「明日（当日＋1日）」に一致する単語数（全レベル横断）。
- **今週の復習予定数**: `next_review_at` の日付が「今日から7日以内（当日を含む当日〜当日＋6日）」に収まる単語数（全レベル横断）。
- **集計元**: `user_progress.next_review_at`（`idx_progress_next_review` を利用）。`next_review_at` が NULL（未学習）の単語は対象外。
- **全レベル共通の明示**: LV詳細画面がどのLVのものであっても、明日／今週の数字はそのLVに絞り込まず全レベルの合算値を表示する（RFP 4.2 注記・受け入れ基準4.2）。
- **表示**: 「明日: {n}語」「今週: {m}語」の2数値を並べて表示する。
- **空状態**: 該当0語でも数値を隠さず「明日: 0語／今週: 0語」と表示する（セクションは常時表示）。
...
3. **復習予定**（全レベル共通の数字）: 明日の復習予定数／今週（今日から7日以内）の復習予定数。算出・表示仕様は §4.6 に従う。
4. **苦手な単語 TOP5**（このLV内）。
...
| 復習予定（明日／今週）を全レベル共通で表示 | §4.6（復習予定の算出・表示仕様＝全レベル横断の明日／今週）、§6.2・§6.5 |
