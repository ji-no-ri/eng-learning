...
```mermaid
flowchart TD
    subgraph Host["開発ホスト（Jotaro: peacock / Windows・Linux）"]
        SRC["ソースコード（このリポジトリ）"]
...
- **ベースイメージ候補**: `ghcr.io/cirruslabs/flutter:<stable>`（Flutter SDK・Dart・Android SDK・JDK を同梱するコミュニティ標準イメージ）。自前構築する場合は Debian/Ubuntu ベースに下表のツールを導入する。
- **同梱ツールとバージョン方針**（具体値は環境構築フェーズで最新安定版に**ピン留め**する。以下は指標）:

| ツール | バージョン方針 | 用途 |
|---|---|---|
| Flutter SDK | stable チャネル（例: 3.x 系の最新安定版） | ビルド・テスト基盤 |
| Dart SDK | Flutter 同梱版 | 言語ランタイム |
| Android SDK | `compileSdk` / `targetSdk` は最新安定 API レベル、`minSdk` はサポート下限を別途決定 | Android ビルド |
| Android Build-Tools / Platform-Tools | Android SDK と整合する最新 | ビルド・`adb` |
| JDK | 17（LTS） | Gradle 実行に必要 |
| Gradle | Android Gradle Plugin と整合するラッパ指定版 | Android ビルド |
...
| `JAVA_HOME` | `/usr/lib/jvm/java-17` | JDK パス |
...
- **ベースイメージ候補**: `budtmo/docker-android` や `cirruslabs` のエミュレータ系イメージ等（system-image + emulator + noVNC を含むもの）。
...
| `ANDROID_API_LEVEL` | 最新安定 API | system-image のバージョン |
...
- **ベースイメージ**: `nginx:stable-alpine`（静的ファイル配信のみ）。
...
環境構築フェーズで `.env`／compose に定義する変数の集約。値は指標であり、実装時にピン留め・確定する。

| 変数 | サービス | 例 | 用途 |
|---|---|---|---|
| `FLUTTER_HOME` | flutter-dev | `/sdks/flutter` | Flutter SDK パス |
| `ANDROID_SDK_ROOT` | flutter-dev / emulator | `/opt/android-sdk` | Android SDK パス |
| `JAVA_HOME` | flutter-dev | `/usr/lib/jvm/java-17` | JDK パス |
...
| `ANDROID_API_LEVEL` | android-emulator | 最新安定 API | system-image バージョン |
...
- **バージョンは実装時ピン留め**: 本書のバージョンは方針・指標であり、環境構築フェーズで最新安定版に固定（再現性のためタグ・SDK バージョンを明示）する。
